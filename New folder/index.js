...
module.exports = client = async (client, mek) => {
	try {
        ...
        const fakethumb = (teks, yes) => {
            client.sendMessage(from, teks, image, {thumbnail:fs.readFileSync('./stik/fake.jpeg'),quoted:mek,caption:yes})
        }
...
switch (command) {
	...
	if (!q)	return reply(`CONTOH PENGGUNANNYA:\n${prefix}reminder text/2s\n\nNOTE: \n*s* - seconds\n*m* - minutes\n*h* - hours\n*d* - days`)
                ...
                await client.sendMessage(from, `── 「 REMINDER 」 ──
...
client.sendMessage(from, media, image, {contextInfo:{mentionedJid: [sender]}, caption: teks})
                        _reminder.splice(reminder.getReminderPosition(sender, _reminder), 1)
                        fs.writeFileSync('./database/reminder.json', JSON.stringify(_reminder))
                        clearInterval(intervRemind)
                    }
...
➸ Pesan: ${messRemind}
➸ Type: ${reminder.getReminderType(sender, _reminder)}`, text, {contextInfo:{mentionedJid: [sender]}})
client.sendMessage(from, fs.readFileSync(media), audio, {contextInfo:{mentionedJid: [sender]}, mimetype: 'audio/mp4', ptt: true, caption: teks})
                        _reminder.splice(reminder.getReminderPosition(sender, _reminder), 1)
                        fs.writeFileSync('./database/reminder.json', JSON.stringify(_reminder))
                        clearInterval(intervRemind)
                    }
...
case 'closegc':
    ...
    case 'linkwa':
            ...
            const tod = await fetchJson(`https://api.dhnjing.xyz/stalk/instagram?user=${q}`)
    ...
    client.sendMessage(from, buff, image, {quoted: mek, caption: tt})
            ...
            case 'githubstalk':
            if(!q) return reply('Usernamenya?')
            const oi = await fetchJson(`https://api.dhnjing.xyz/stalk/github?user=dehan-j1ng`)
    ...
    reply(mm)
            ...
            case 'tiktokstalk':
            if(!q) return reply('Usernamenya? ')
            var i = await fetchJson(`https://api.dhnjing.xyz/stalk/tiktok?user=@${q}`)
